面向桌面发行版的 PaX/Grsecurity 配置详细注释与评论
==============================================
...
[\*] Forcibly initialize local variables copied to userland (CONFIG_PAX_MEMORY_STRUCTLEAK)

选择 Y，PaX 会在将一些内核局部变量在拷贝到用户空间之前，以 0 初始化这些
变量。这在单核系统上的开销小于 1%。

[\*] Prevent invalid userland pointer dereference (CONFIG_PAX_MEMORY_UDEREF)
...
[\*] Prevent various kernel object reference counter overflows (CONFIG_PAX_REFCOUNT)
...
[\*] Harden heap object copies between kernel and userland (CONFIG_PAX_USERCOPY)
...
[  ] Prevent various integer overflows in function size parameters CONFIG_PAX_SIZE_OVERFLOW)
...
[\*] Generate some entropy during boot and runtime (CONFIG_PAX_LATENT_ENTROPY)
...
[  ] Prevent code reuse attacks (CONFIG_PAX_RAP)
...
[  ] Deny reading/writing to /dev/kmem, /dev/mem, and /dev/port (CONFIG_GRKERNSEC_KMEM)
...
[\*] Harden BPF interpreter (CONFIG_GRKERNSEC_BPF_HARDEN)
...
选择 Y，这会让 PaX 禁止普通用户使用 PERF_EVENTS 性能计数器。PERF_EVENTS 计数器对于程序的
...
某个关键的内核内存地址并发动攻击。获取加载模块信息以及内核符号的程序将只限于拥有 CAP_SYS_MODULE
...
需要更多信息才能攻击内核数据，加大攻击难度。随机种子将保存在 tools/gcc/randomize_layout_seed.h，
