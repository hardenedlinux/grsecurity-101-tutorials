...
相似的，4.9 的代码也做了 thread_info 的分离和分配虚拟地址连续的内存块（基于Grsec/Pax?），不同点在于 4.9 直接分配出物理上不连续的內核栈。  
